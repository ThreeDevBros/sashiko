...
patch_app_delegate() {
  ...
  echo "🔧 Injecting push notification setup into AppDelegate.swift..."
  python3 - "$APPDELEGATE" "$SWIFT_TEMPLATE" <<'PY'
from pathlib import Path
import re
import sys
...
helper_marker = "// MARK: - Push Notification Setup (generated by scripts/rebuild-ios.sh)"
if helper_marker not in text:
    helper_lines = helper.splitlines()
    while helper_lines and (helper_lines[0].startswith("import ") or helper_lines[0].strip() == ""):
        helper_lines.pop(0)
